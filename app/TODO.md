...
### 7) Stacks
- STK-1 [mvp] Make/Unstack from selection; order by Y then X — owner: niko — status: done (completed 2025-10-21) ✅ **COMPLETED**: Complete stack functionality implementation including: 1) **Stack Creation**: Y-then-X ordering algorithm that arranges selected items vertically by primary Y-coordinate, then horizontally by X-coordinate within each row, 2) **Stack Destruction**: Grid layout restoration that maintains relative positioning while unstacking, 3) **Keyboard Shortcuts**: Ctrl/Cmd+S for stack creation, Ctrl/Cmd+Shift+S for unstack operations, 4) **Full Undo/Redo Support**: Complete command pattern integration with CreateStackCommand and DestroyStackCommand, 5) **Selection System Integration**: Seamless integration with existing selection mechanisms and visual feedback, 6) **Comprehensive Testing**: All functionality tested and builds successfully. (Commit: b272c9b)
- STK-2 [mvp] Enter/Cmd+Enter add sibling; Tab indent/outdent — owner: niko — status: done (completed 2025-10-21) ✅ **COMPLETED**: Complete stack manipulation shortcuts implementation including: 1) **Sibling Creation**: Enter key adds sibling below selected note, Cmd/Ctrl+Enter adds sibling above, with proper stack ordering and position calculation, 2) **Indent/Outdent**: Tab key indents selected note, Shift+Tab outdents, with hierarchical level management, 3) **Smart Context Detection**: Shortcuts only work for notes in stacks, falling back to normal editing behavior for non-stacked notes, 4) **Command Pattern Integration**: Added AddSiblingNoteCommand and ChangeIndentCommand with full undo/redo support, 5) **Stack Integration**: Updated Canvas component to receive and use stack information for all operations, 6) **Comprehensive Testing**: All functionality tested and builds successfully. (Commit: 332f2d5)
- STK-3 [mvp] Align/Distribute within stack; same width/height — owner: claude — status: in_progress
